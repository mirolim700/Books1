...
class Database():
	def __init__(self):
		self.conn = sqlite3.connect("books1.db")
	...
	def insert_products(self,file_name,category_id,file_des,file_photo,file_id):
		self.cur.execute(f"""INSERT into products(file_name,category_id,file_des,file_photo,file_id) values("{file_name}","{category_id}","{file_des}","{file_photo}","{file_id}")""" )
		return self.conn.commit()
